...
async def test_evaluate_all_true_positives():
    ...
    actual_results = await evaluate(claims, ground_truths, model_name=model_name, client=client)
...
async def test_evaluate_with_false_positives():
...
async def test_evaluate_with_historical_facts():
...
async def test_evaluate_with_scientific_facts():
...
async def test_evaluate_with_environmental_statements():
...
async def test_evaluate_with_health_nutrition_statements():
...
async def test_evaluate_with_technological_advancements():
...
async def test_evaluate_with_economic_statements():
